...
-- A browser session for the profile page, and deliberately not a refresh token.
--
-- The OAuth callback's own comment explains why the browser half of sign-in
-- carries nothing worth stealing: there is no token in a tab to take. Putting
-- a refresh token in a cookie to build a profile page would throw that away.
-- So this is a separate credential with a much smaller blast radius: it reads
-- one account's email, plan and usage, it cannot mint a claim, it cannot buy
-- anything, and it expires in half a day.
